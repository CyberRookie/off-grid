<?php
  ...
  /**
   * 12/11/21 Modified to reflect fields in the users off-grid database
   * @param Array $data
   * @return Array | void
  ...
  function Login(array $data)
  {
    ...
    if (!$Email_check['status']) {
      ...
      if (password_verify($Password, $Email_check['data']['password'])) {
        ...
        header("Location: ../Off-GridTemplate.html");  //changed to redirect to the template file after logging in.
      }
  ...
  */
  ...
  {
    $dbHandler = DbHandler();
    $statement = $dbHandler->prepare("SELECT `full_name`, `username`, `email`, `password` FROM `users` WHERE `email` = :email");
